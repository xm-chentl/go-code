...
var (
	//formatQuote = `"github.com/xmleban/%s/api%s"`
...
func GenerateMatedata(apiDir, quota string) (err error) {
	...
	for index, info := range apiInfoArray {
		...
		importQuote := fmt.Sprintf(`"%s/api%s"`, quota, apiInfoArray[index].target)
